                ...
                <div className="text-sm">
                  ...
                  <div className="space-y-1">
                    {currentPlayer.buyIns.map((buyIn, index) => (
                      <div key={index} className="flex justify-between items-center bg-gray-700 px-2 py-1 rounded">
                        ...
                        <span className={`text-xs px-2 py-1 rounded ${buyIn.type === 'bit' ? 'bg-green-600' : 'bg-blue-600'
                          }`}>
                          {buyIn.type === 'bit' ? 'BIT' : 'מזומן'}
                        </span>
                      </div>
                    ))}
                  </div>
                  <div className="mt-3 p-2 bg-gray-700 rounded">
                    ...
                    <div className="text-xs text-gray-300 mt-1">
                      <span>BIT: ₪{getBuyInsByType(currentPlayer, 'bit')}</span>
                      <span className="ml-3">מזומן: ₪{getBuyInsByType(currentPlayer, 'cash')}</span>
                    </div>
                  </div>
                </div>
              )}
                ...
                {players.map((player, index) => (
                  <div key={index} className="p-3 bg-gray-700 rounded transition-all">
                    ...
                    <div className="text-xs text-gray-400 flex justify-between mb-2">
                      <div>
                        <span>BIT: ₪{getBuyInsByType(player, 'bit')}</span>
                        <span className="ml-2">מזומן: ₪{getBuyInsByType(player, 'cash')}</span>
                      </div>
                      ...
                      <div className="mt-2 pt-2 border-t border-gray-600">
                        <button
                          onClick={() => {
                            ...
                            if (isOpening) {
                              setAdminBuyInType(prev => ({ ...prev, [player.name]: 'bit' })); // Default to BIT
                            }
                          }}
                            ...
                            <div>
                              ...
                              <div className="flex gap-1 mb-1">
                                <button
                                  ...
                                  className={`text-[10px] px-2 rounded ${adminBuyInType[player.name] === 'bit' || !adminBuyInType[player.name] ? 'bg-green-600' : 'bg-gray-600'}`}
                                >
                                  BIT
                                </button>
                                <button
                                  ...
                                  className={`text-[10px] px-2 rounded ${adminBuyInType[player.name] === 'cash' ? 'bg-blue-600' : 'bg-gray-600'}`}
                                >
                                  מזומן
                                </button>
                              </div>
